...
type AtLister interface {
	...
	List(selector labels.Selector) (ret []*v1.At, err error)
...
// List lists all Ats in the indexer.
func (s *atLister) List(selector labels.Selector) (ret []*v1.At, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.At))
	})
	return ret, err
}
...
// List lists all Ats in the indexer for a given namespace.
func (s atNamespaceLister) List(selector labels.Selector) (ret []*v1.At, err error) {
...
// Get retrieves the At from the indexer for a given namespace and name.
func (s atNamespaceLister) Get(name string) (*v1.At, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("at"), name)
	}
	return obj.(*v1.At), nil
}
